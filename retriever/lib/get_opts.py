...
new_json_parser = subparsers.add_parser(
    "new_json", help="CLI to create retriever datapackage.json script"
)
edit_json_parser = subparsers.add_parser(
    "edit_json", help="CLI to edit retriever datapackage.json script"
)
delete_json_parser = subparsers.add_parser(
    "delete_json", help="CLI to remove retriever datapackage.json script"
)
...
reset_parser.add_argument(
    "scope", help="things to reset: all, scripts or data"
).completer = ChoicesCompleter(script_list + ["all", "scripts", "data"])
...
install_parser.add_argument(
    "--not-cached", help="overwrites local cache of raw data", action="store_true"
)
download_parser.add_argument(
    "dataset", help="dataset name"
).completer = ChoicesCompleter(script_list)
download_parser.add_argument(
    "-b",
    "--bbox",
    nargs=4,
...
ls_parser.add_argument(
    "-l", help="search datasets with specific license(s)", nargs="+"
).completer = ChoicesCompleter(list(licenses_options))
ls_parser.add_argument(
    "-k", help="search datasets with keyword(s)", nargs="+"
).completer = ChoicesCompleter(list(keywords_options))
ls_parser.add_argument(
...
install_subparsers = install_parser.add_subparsers(
    help="engine-specific help", dest="engine"
)

for engine in engine_list:
    if (
        engine.name == "Download Only"
    ):  # skip the Download engine and just add attributes
        pass
    else:
        engine_parser = install_subparsers.add_parser(
